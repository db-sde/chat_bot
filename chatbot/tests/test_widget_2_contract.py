...
def _module_source(name: str) -> str:
    return (WIDGET_DIR / "src" / name).read_text(encoding="utf-8")
...
def test_widget_source_modules_preserve_bootstrap_and_transport_boundaries() -> None:
    module_names = {
        "widget.js",
        "api.js",
        "state.js",
    ...
    sources = {name: _module_source(name) for name in module_names}

    bootstrap_functions = set(
        re.findall(r"^  (?:async\s+)?function\s+([A-Za-z_$][\w$]*)\s*\(", sources["widget.js"], re.MULTILINE)
    )
    assert bootstrap_functions == {"cleanPageType", "buildWidget"}
    assert "@include api.js" in sources["widget.js"]
    assert "@include renderer.js" in sources["widget.js"]
    assert "@include actions.js" in sources["widget.js"]
    assert "@include tools.js" in sources["widget.js"]

    for name, source in sources.items():
        if name != "api.js":
            assert not re.search(r"\bfetch\s*\(", source), f"raw fetch leaked into {name}"

    for name in ("api.js", "state.js", "config.js"):
        assert "document." not in sources[name]

    generated = _source("widget.js")
    assert generated.startswith(
        "/**\n"
        " * --------------------------------------------------\n"
        " * AUTO-GENERATED FILE\n"
    )
    assert "DO NOT EDIT THIS FILE." in generated[:300]
    assert "@include" not in generated

    build_source = _source("build.mjs")
    assert "readdir(sourceDirectory" in build_source
    assert 'process.argv.includes("--watch")' in build_source
    assert "watchFile(" in build_source
...
def test_guided_actions_do_not_enter_typed_chat_transport() -> None:
    ...
    request_chat = _function_source(source, "requestChat")
    assert "requestChat(body, controller.signal)" in send_message
    assert "fetch(`${apiBase}/chat`" in request_chat
...
def test_premium_admissions_ui_contracts_are_present() -> None:
    ...
    assert "db-msg" in create_message
    assert "db-bubble-${role}" in create_message
    assert "db-widget__avatar--message" not in create_message
    assert "db-widget__message-row--grouped" not in create_message
