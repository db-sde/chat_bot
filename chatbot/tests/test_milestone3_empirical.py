...
def test_widget_parameter_tolerance():
    ...
    with TestClient(app) as client:
        ...
        r_query = client.post(
            "/chat?widget_id=abc&timestamp=123456789&source=widget",
            json={"message": "tell me about MBA", "session_id": "test_tolerance_query"},
        )
...
def test_advisory_classification_routing():
    ...
    with TestClient(app) as client:
        ...
        r2 = client.post(
            "/chat",
            json={"message": "tell me the best mba courses", "session_id": "test_advisory_route_2"},
        )
        ...
        r4 = client.post(
            "/chat",
            json={"message": "Show MBA specializations", "session_id": "test_advisory_route_4"},
        )
...
def test_lead_funnel_precedence():
    ...
    with TestClient(app) as client:
        ...
        assert r1.status_code == 200
        payloads1 = extract_stream_payloads(r1.text)
        first_text = payloads1[-1]["text"].lower()
        assert "name" in first_text or "counsellor" in first_text

        # Step 2: Send a product query for a Catalog V3 LPU program.
        r2 = client.post(
        ...
        assert "157,000" in text2 or "fee" in text2.lower()

        has_lpu_context = (
            "Lovely Professional University" in text2
            or "LPU" in text2
            or any(
                "Lovely Professional University" in chip or "LPU" in chip
                for chip in payloads2[-1].get("suggested_chips", [])
            )
        )
