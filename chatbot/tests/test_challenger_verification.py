...
def test_advisory_classification():
    ...
    with TestClient(app) as client:
        ...
        res_b = client.post(
            "/chat",
            json={"message": "tell me the best mba courses", "session_id": "test_adv_b"}
        )
        ...
        # Expected: Since it has no specific university/category/specialization name, it routes to fallback/unrelated.
        res_c = client.post(
            "/chat",
...
def test_lead_funnel_precedence():
    ...
    with TestClient(app) as client:
        ...
        res_start = client.post(
            "/chat",
            json={"message": "Request Callback", "session_id": session_id}
        )
        ...
        # 2. Send product query: "what is the fee for LPU MBA?"
        res_query = client.post(
            "/chat",
            json={"message": "what is the fee for LPU MBA?", "session_id": session_id}
        )
        assert res_query.status_code == 200
        payload_query = parse_sse(res_query.text)

        # 3. Verify it exited the funnel and displayed the course fee
        # The response text should describe fees for the MBA course
        assert "fee" in payload_query.get("text", "").lower()
        # Verify the suggested chips refer to Lovely Professional University (LPU)
        assert any("Lovely Professional University" in chip for chip in payload_query.get("suggested_chips", []))
        
        # Verify that it is no longer asking for lead info in the next turn (it's back to normal chat)
        ...
        # Should route to normal chat/fallback/discovery instead of saying "Thanks, I've saved your phone..."
