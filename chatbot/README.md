...
record, tolerates partial catalog content, and progressively captures CRM leads.
...
- `GROQ_API_KEY`: optional fast-model intent classification.
...
- `CRM_WEBHOOK_URL` and `CRM_WEBHOOK_SECRET`: optional CRM destination. Each newly captured
  lead field schedules a background update. Exhausted retries are written to
  `DEAD_LETTER_PATH`.

See `.env.example` for model names, timeouts, circuit-breaker thresholds, lead timing, and
